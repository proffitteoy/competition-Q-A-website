# 学院竞赛管理与问答平台文档

## 文档定位

本目录不再作为通用提示词模板集合使用，当前阶段改为本项目的产品与实施文档入口。

项目目标：

**围绕学院竞赛场景，建立从发布、咨询、报名、审核、导出到资料沉淀的一整套闭环平台。**

当前优先级不是直接做完整系统，而是先固定以下内容：

1. 页面清单
2. 每页信息架构
3. 用户操作路径
4. 关键数据状态
5. 管理员工作流
6. 视觉风格规范
7. Mock 数据版前端范围

## 建议阅读顺序

1. [项目定位与需求基线](./项目定位与需求基线.md)
2. [技术选型与模板映射](./技术选型与模板映射.md)
3. [组件选型与分层策略](./组件选型与分层策略.md)
4. [页面结构与用户流程](./页面结构与用户流程.md)
5. [数据层与后端架构](./数据层与后端架构.md)
6. [报名表数据模型与导出约束](./报名表数据模型与导出约束.md)
7. [状态流转矩阵](./状态流转矩阵.md)
8. [MVP 与迭代路线](./MVP与迭代路线.md)
9. [视觉与交互规范](./视觉与交互规范.md)
10. [当前已实现功能清单](./当前已实现功能清单.md)
11. [首页门面化改版方案（专项）](./首页门面化改版方案.md)
12. [名人堂 + 个人主页实现方案（专项）](./plan-hall-of-fame.md)
13. [个人中心与名人堂联动方案（待评审）](./个人中心与名人堂联动方案.md)
14. [生产数据库迁移与回滚手册](./生产数据库迁移与回滚手册.md)
15. [端到端回归执行清单](./端到端回归执行清单.md)
16. [调试样例与管理员入口](./调试样例与管理员入口.md)
17. [问题修复记录（2026-05-01）](./问题修复记录-2026-05-01.md)

## 当前结论

- 平台定位不是泛论坛，也不是普通官网，而是“学院竞赛管理与问答平台”。
- 业务核心优先级为：`报名与审核` > `竞赛门户与通知` > `围绕竞赛的问答沉淀` > `精细化权限与统计扩展`。
- 第一版应先完成公开页、报名链路、管理员审核与导出，不应一开始就铺开社区化功能。
- 问答系统是服务竞赛的辅助模块，不应主导首页结构和信息层级。
- 技术路线优先选择 `Next.js + Tailwind CSS + shadcn/ui`。
- 模板策略为：`ShadcnStore Dashboard + Landing Template` 做主骨架，`Launch UI` 只做学院竞赛门户首页与展示区补强。
- 组件策略为：`shadcn/ui` 做底座，业务表格/表单优先复用主模板现成依赖，展示层优先参考本地 `Launch UI`，不堆多套完整 UI 体系。
- 阶段 2 的默认后端路线已固定为：`Next.js 单体 + PostgreSQL + Drizzle + Auth.js + S3 兼容对象存储`。
- 当前代码骨架已落到仓库根目录的 `src/` 与 `public/`，外部开源目录转为上游来源和待清理对象。
- 根项目路由已完成首轮去模板化清理，主业务路由聚焦竞赛门户、报名、登录和管理员工作台，同时保留认证与错误页基础路由。

## 当前实现进展（阶段 2）

- 已引入 `PostgreSQL + Drizzle ORM + Auth.js` 基础工程结构：
  - `src/lib/db/`：数据库配置、schema、seed 脚本
  - `src/lib/auth/`：Auth.js 配置与会话读取
  - `src/server/permissions/`：比赛作用域权限检查
  - `src/server/services/`：报名与比赛状态流转服务
  - `src/actions/`：Server Actions 入口（学生提交、我的报名、管理员审核）
- 后台报名工作台已补齐：
  - 筛选（关键词 / 状态 / 比赛）
  - 批量处理（通过 / 驳回 / 取消）
  - 导出 `CSV / XLSX`
- `/admin/schedule` 已改为读取数据层，而非直接读取 mock 常量。

### 本地命令

- `npm run db:generate`：生成 migration（Drizzle）
- `npm run db:migrate`：执行 migration
- `npm run db:migrate:sql`：执行手工 SQL 迁移（受限环境兜底）
- `npm run db:push`：直接推送 schema（开发环境）
- `npm run db:seed`：写入 MVP 初始数据
- `npm run seed:debug`：写入扩展调试样例（等价于 `db:seed`）
- `npm run test:permissions`：执行权限越权回归测试
- `npm run admin:create-debug`：创建/刷新调试管理员账号（`super_admin`）

### 无数据库时的开发兜底（2026-04）

- 若未配置 `DATABASE_URL`（且未配置 `PGHOST/PGPORT/PGDATABASE/PGUSER/PGPASSWORD`），前台与查询类页面会自动回退到 `src/lib/mock-data.ts` 的只读数据。
- 写操作（如报名提交、审核、创建比赛）在无数据库模式下会返回明确错误，避免误以为数据已持久化。
- 需要验证真实报名与审核链路时，仍应先完成数据库配置与迁移/seed。

## 与旧文档的关系

- `docs/AGENTS.md` 已切换为当前项目在 `docs/` 目录下的协作约束。
- 目录中保留的 `通用项目架构模板.md`、`skills.md`、`代码组织.md`、`代码审计.md`、`强前置条件约束.md` 等文件，目前只可视为历史参考材料。
- 后续开发与设计，以上面 9 份项目文档为主，不再以通用模板内容作为实现依据。
